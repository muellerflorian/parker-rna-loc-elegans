...
def process_folder_fiji(path_open,channels,img_ext,annot_ext,masks_save):
    ...
    folderImporter = annotationImporter.FolderImporter(channels=channels, data_category={'train':'train','test':'test'},img_ext=img_ext,annot_ext =annot_ext)
        ...
        for key_channel, channel in annotDict['config']['channels'].items():
           ...
           for key_file, v in maskDict.items():
               for key_mask_sel in masks_save[key_channel]:
                   ...
                   file_name_save = os.path.join(path_save_categ, '{}_{}.png'.format(file_base,key_mask_sel))
